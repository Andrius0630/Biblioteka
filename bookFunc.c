...
void readFile(char *filename, unsigned short *lineCount, char ***buffer) {
    unsigned short i = 0, size = 10;
    char c = 1;
    char *tmp = NULL;
    char *line = NULL;
    FILE *fptr = fopen(filename, "r");
    if (fptr == NULL) {printf("Error! File %s is not found.\n", filename); free(filename); exit(1);}
    while (1)
    {
        size = 10;
        i = 0;
        ...
        while ((c = fgetc(fptr)) != '\n' && c != EOF)
        {
            if(i >= size - 1) {
                ...
                if(tmp == NULL) {free(line); fclose(fptr); exit(1);}
                line = tmp;
            }
            line[i++] = c;
        }
        line[i] = '\0';
        if(c == EOF && i == 0) {free(line); exit(1);}
        *buffer = realloc(*buffer, sizeof(char *) * (*lineCount + 1));
        if (*buffer == NULL) {free(line); fclose(fptr); exit(1);}
    ...
    for (i = 0; i < lineCount; i++)
    {
        for (j = 0; j < lineCount; j++)
        {
            if (strcmp(books[i].author, books[j].author) == 0 && strcmp(books[i].name, books[j].name) == 0 && books[i].mentioned == 0)
            {
                ...
                if (fptr != NULL) {
                    fprintf(fptr, "%s,%s,%d,%d\n", books[j].author, books[j].name, books[j].date, books[j].pages);
                    fclose(fptr);
                    books[i].mentioned = 1;
                } else exit(1);
